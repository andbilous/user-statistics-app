...
let initialqueryUsers = `SELECT DISTINCT ID id FROM users
           ORDER BY id`;

let initialqueryUsersStatistics = `SELECT DISTINCT ID id FROM users_statistic
           ORDER BY id`;
...
db.all(selectFromUsers, [], (err, rows) => {
  if (err) {
    throw err;
  }
  rows.forEach((row) => {
...
db.all(selectFromUserStatistic, [], (err, rows) => {
  if (err) {
    throw err;
  }
  rows.forEach((row) => {
